 ...
 * http://muhammad.com/
 */
data class Event<out T>(val status: Status, val data: T?, val error: Error?, var message : String) {
    ...
    companion object {
        ...
        fun <T> error(error: Error?, message: String): Event<T> {
            return Event(Status.ERROR, null,error,message)
        }
    }
}
